...
# With docker for mac and docker rootless, root will own the mount point. Add --non-unique so that evap is an alias to root (uid=0) in this case
...
useradd -m -u $(stat -c "%u" "$MOUNTPOINT/evap") -s /bin/bash evap --non-unique
# allow ssh login
